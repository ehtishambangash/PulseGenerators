...
check_row1
		...
		LDR	R4,=(GPIO1DATA) 
		MOVS R5, #0x07		;set 1st row = 0 and (2,3,4) = 1
		STR R5, [R4]
		...
		ANDS R5, R5, R3		;find inoup column
		MOVS R5, R5, LSR #1  ; shift right one bit
		
		;;;;;;;;;; check first column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0x07  		;0111 compare if the first column is 0, "1" on the keypad is pressed. 
		BEQ _col1_row1
		
		;;;;;;;;;; check second column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xB  		;1011 compare if the second column is 0, "2" on the keypad is pressed. 
		BEQ _col2_row1
		
		;;;;;;;;;; check third column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xD  		;1101 compare if the third column is 0, "3" on the keypad is pressed. 
		BEQ _col3_row1
		
		;;;;;;;;;; check fourth column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xE  		;1110 compare if the fourth column is 0, "A" on the keypad is pressed. 
		BEQ _col4_row1
...
check_row2
		...
		LDR	R4,=(GPIO1DATA) 
		MOVS R5, #0xB		;set 2nd row = 0 and (1,3,4) = 1
		STR R5, [R4]
		...
		CMP R5, #0x07  		;0111 compare if the first column is 0, "4" on the keypad is pressed. 
		BEQ _col1_row2
		
		;;;;;;;;;; check second column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xB  		;1011 compare if the second column is 0, "5" on the keypad is pressed. 
		BEQ _col2_row2
		
		;;;;;;;;;; check third column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xD  		;1101 compare if the third column is 0, "6" on the keypad is pressed. 
		BEQ _col3_row2
		
		;;;;;;;;;; check fourth column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xE  		;1110 compare if the fourth column is 0, "B" on the keypad is pressed. 
		BEQ _col4_row2
...
check_row3
		...
		LDR	R4,=(GPIO1DATA) 
		MOVS R5, #0xD		;set 3rd row = 0 and (1,2,4) = 1
		STR R5, [R4]
		...
		CMP R5, #0x07  		;0111 compare if the first column is 0, "7" on the keypad is pressed. 
		BEQ _col1_row3
		
		;;;;;;;;;; check second column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xB  		;1011 compare if the second column is 0, "8" on the keypad is pressed. 
		BEQ _col2_row3
		
		;;;;;;;;;; check third column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xD  		;1101 compare if the third column is 0, "9" on the keypad is pressed. 
		BEQ _col3_row3
		
		;;;;;;;;;; check fourth column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xE  		;1110 compare if the fourth column is 0, "C" on the keypad is pressed. 
		BEQ _col4_row3
...
check_row4
		...
		LDR	R4,=(GPIO1DATA) 
		MOVS R5, #0xE		;set 4th row = 0 and (1,2,3) = 1
		STR R5, [R4]
		...
		CMP R5, #0x07  		;0111 compare if the first column is 0, "*" on the keypad is pressed. 
		BEQ _col1_row4
		
		;;;;;;;;;; check second column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xB  		;1011 compare if the second column is 0, "0" on the keypad is pressed. 
		BEQ _col2_row4
		
		;;;;;;;;;; check third column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xD  		;1101 compare if the third column is 0, "#" on the keypad is pressed. 
		BEQ _col3_row4
		
		;;;;;;;;;; check fourth column ;;;;;;;;;;;;;;;;;;
		
		CMP R5, #0xE  		;1110 compare if the fourth column is 0, "D" on the keypad is pressed. 
		BEQ _col4_row4
...
;;;;;;;;;;;;;;;;;; keypad detection ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	;then store 1 to R5
    MOVS R5, #(0x1);
	;Load a bit pattern 0x1 into R5  and shift left by 8 position to set bit 1 enable write mode
	MOVS R5, R5, LSL #8
	ORRS R5, R0
    ; Store the new value of R5 into GPIO1DIR;
    STR R5, [R4];
	...
	MOVS R5, #0x1			;move value 1 to R5
	MOVS R5, R5, LSL #31	;Load R5 with all zeros except bit-32 set, lowest priority
	STR R5, [R4]			;store the value of R5 into ISER of NVIC
	...
	MOVS R5, #0x1			;move value 1 to R5
	MOVS R5, R5, LSL #31	;Load R5 with all zeros except bit-31 set, lowest priority
	STR R5, [R4]			;store the value of R5 into ICPR
	
	
	;;clear the rows for the next keypad press
	;set GPIO1DATA low
	LDR R4, =(GPIO1DATA); GPIO1DATA Base + 0x3FFC, address 0x5001 3FFC, Table 174, page 192
	; Load the value of GPIO1DATA into R5
    LDR R5, [R4];
	; Load the value 0 
	MOVS R5, #(0x0);
	; Store the value of R5 into GPIO1DATA
	STR R5, [R4];
